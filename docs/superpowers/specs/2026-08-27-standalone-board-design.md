...
```
id             BIGINT       PK
email          VARCHAR(255) NOT NULL UNIQUE   -- 계정의 열쇠
...
3. 이메일이 없거나 **미검증**이면 → 로그인을 완료하지 않는다. 이메일 입력 화면을 보여주고,
   입력받은 주소로 `verify_email` 토큰을 보낸다. 프로바이더 프로필은 짧은 TTL 로 세션에만
   둔다. **링크 클릭 시점에** 연결 또는 생성을 확정한다.
...
응답으로 가입 여부가 드러나지 않게 한다. 링크에서 새 비밀번호를 설정하면 그 회원의 모든
세션을 무효화한다.
