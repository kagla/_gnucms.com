...
설정은 세 겹으로 겹친다. 뒤가 앞을 덮는다.

1. `Config::defaults()` — 코드에 있는 기본값. `bootstrap_admin` 은 기본이 닫힘(`null`)이다
2. `config/config.php` — 설치 마법사가 생성한다. 생성된 시크릿과 비밀번호 해시가 여기 있다
3. `.env` (프로젝트 루트) — 환경마다 달라지는 값
4. 진짜 환경변수 — 도커·패널이 주입한다. `.env` 를 덮는다

`.env` 를 도입한 이유는 값 하나를 바꾸려고 PHP 파일을 열지 않기 위해서다. 문법 오류
하나가 사이트 전체를 죽이는 파일과, 값만 적는 파일은 위험도가 다르다.

`config.php` 를 없애지 않은 이유는 두 가지다. 설치 마법사가 만들어 낸 시크릿과 해시를
담을 곳이 필요하고, 문서 루트를 옮길 수 없는 호스팅에서 `.env` 는 평문으로 노출될 수
있지만 PHP 파일은 실행되어 아무것도 내보내지 않는다. 그래서 `.env` 는 선택이고,
있으면 이긴다. `public/.htaccess` 에 `.env` 차단 규칙을 함께 둔다.

빈 값(`KEY=`)은 "설정하지 않음" 으로 본다. 아래 층의 값을 그대로 둔다. `.env.example`
을 복사해 필요한 줄만 채우는 것이 정상적인 사용법인데, 채우지 않은 줄이 설치 마법사가
만들어 낸 시크릿을 지워 버리면 안 되기 때문이다. 아래 층의 값을 실제로 비우려면
`KEY=null` 이라고 쓴다.

`.env` 파서는 셸 문법을 흉내 내지 않는다. 인라인 주석, 변수 치환, 여러 줄 값을
지원하지 않는다. 인라인 주석을 지원하면 `DB_PASSWORD=p@ss#word` 가 조용히 잘린다.
형식이 어긋난 줄은 건너뛰지 않고 오류로 만든다.

`.env` 에서 읽는 이름은 화이트리스트다. 목록에 없는 이름은 무시한다. 요청 헤더는 언제나
`HTTP_` 접두사가 붙어 `$_SERVER` 에 들어오므로 정확한 이름만 찾는 조회로는 밖에서
설정을 밀어 넣을 수 없다.

`config.php` 없이 `.env` 만으로도 배포할 수 있다. 그래서 "설치되었는가" 의 판정은
`config.php` 의 존재가 아니라 `db.dsn` 이 정해졌는가로 한다.
